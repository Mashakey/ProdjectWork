...
public class RectangularWallManager : RoomWallManager
{
	[SerializeField]
	Text RoomAreaField;
	...
	float CalculateArea()
	{
		return (walls[0].GetLengthInCm() * walls[1].GetLengthInCm());
	}
	...
	public override RoomData CreateRoomDataFromConstructor()
	{
		...
		rectangleRoom.Type = RoomType.Rectangle;
		rectangleRoom.Height = RoomHeight;
		Debug.LogWarning("ROOM HEIGHT IN RECT = " + RoomHeight);
		...
		rectangleRoom.RoomCorners.Add(new Vector2(walls[1].GetLengthInM(), 0));
	...
	WindowData CreateWindowData(WallData wallData, Vector3 windowPosition)
	{
		...
		Debug.LogWarning("Window position " + window.Position);
	...
	DoorData CreateDoorData(WallData wallData, Vector3 doorPosition)
	{
		...
		Debug.LogWarning("Door position " + door.Position);
		return (door);
	}
	...
	public override Vector3 SetCameraPositionOnMiddle()
	{
		...
		//Camera.main.transform.localPosition = middleRoomPosition;
